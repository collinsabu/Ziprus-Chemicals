...
const CrudeEntriesList = () => {
  ...
  useEffect(() => {
    const fetchEntries = async () => {
      const res = await fetch(`/api/crudeEntries?month=${month + 1}&year=${year}`);
        ...
        <ul>
          {entries.map((entry) => (
            <li key={entry._id} className="mb-2">
              <Link href={`/viewreport/${entry._id}`} className="block p-4 bg-white hover:bg-base_text rounded-lg">
                <p>Date: {entry.date}</p>
                <p>Material Kind: {entry.materialKind}</p>
...
export default CrudeEntriesList;
