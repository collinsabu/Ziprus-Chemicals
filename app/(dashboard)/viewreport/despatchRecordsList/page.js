// src/app/despatchRecordsList/page.js
...
const DespatchRecordsList = () => {
  ...
  useEffect(() => {
    const fetchEntries = async () => {
      const res = await fetch(`/api/despatchRecords?month=${month + 1}&year=${year}`);
  ...
  return (
    <main className="bg-base_two mb-10 pt-40">
      ...
      <h1 className="text-2xl font-bold text-white mb-4">Despatch Record List</h1>
        ...
        <ul>
          {entries.map((entry) => (
            <li key={entry._id} className="mb-2">
              <Link href={`/viewreport/despatchRecordsList/${entry._id}`} className="block p-4 bg-white hover:bg-base_text rounded-lg">
                <p>Date: {entry.date}</p>
                <p>Time: {entry.time}</p>
...
export default DespatchRecordsList;
