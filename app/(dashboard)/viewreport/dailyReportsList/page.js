// src/app/dailyReportsList/page.js
...
const DailyReportsList = () => {
  ...
  useEffect(() => {
    const fetchEntries = async () => {
      const res = await fetch(`/api/dailyReports?month=${month + 1}&year=${year}`);
  ...
  return (
    <main className="bg-base_two">
      ...
      <h1 className="text-2xl font-bold text-white mb-4">Daily R. List</h1>
        ...
        <ul>
          {entries.map((entry) => (
            <li key={entry._id} className="mb-2">
              <Link href={`/viewreport/dailyReportsList/${entry._id}`} className="block p-4 bg-white hover:bg-base_text rounded-lg">
                <p>Date: {entry.date}</p>
                <p>Ideas: {entry.ideas}</p>
