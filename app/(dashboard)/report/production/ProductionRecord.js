...
export default function ProductionRecord() {
  ...
  const [amountPerBag, setAmountPerBag] = useState("");
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (totalProduce <= 0 || amountPerBag <= 0) {
      toast.error("Total Produce and Amount per Bag must be greater than 0");
      return;
    }
    setIsLoading(true);

    const totalProductionAmount = totalProduce * amountPerBag;
    ...
    try {
      ...
      if (json.error) {
        toast.error(`Error: ${json.error}`, { autoClose: 5000 });
      } else {
        toast.success("Record submitted successfully!", { autoClose: 3000 });
