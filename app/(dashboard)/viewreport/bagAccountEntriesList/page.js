...
const BagAccountEntriesList = () => {
  ...
  return (
    <main className="bg-base_two">
      ...
      <h1 className="text-2xl font-bold text-white mb-4">Empty Bag Stock List</h1>
        ...
        <ul>
          {entries.map((entry) => (
            <li key={entry._id} className="mb-2">
              <Link href={`/viewreport/bagAccountEntriesList/${entry._id}`} className="block p-4 bg-white hover:bg-base_text rounded-lg">
                <p>Date: {entry.date}</p>
                <p>Time: {entry.time}</p>
                <p>Purchase: {entry.purchase}</p>
              </Link>
            </li>
          ))}
        </ul>
