// src/app/morningReportsList/page.js
...
const MorningReportsList = () => {
  ...
  useEffect(() => {
    const fetchEntries = async () => {
      const res = await fetch(`/api/morningReports?month=${month + 1}&year=${year}`);
        ...
        <ul>
          {entries.map((entry) => (
            <li key={entry._id} className="mb-2">
              <Link href={`/viewreport/morningReportsList/${entry._id}`} className="block p-4 bg-white hover:bg-base_text rounded-lg">
                <p>Date: {entry.date}</p>
                <p>Plan Activities: {entry.planActivities}</p>
...
export default MorningReportsList;
