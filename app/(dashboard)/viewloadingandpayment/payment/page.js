...
const CustomerPaymentList = () => {
  ...
  const [date, setDate] = useState({
    month: new Date().getMonth() + 1, // Start with current month (1-based)
  ...
  useEffect(() => {
    const fetchEntries = async () => {
      try {
        const res = await fetch(`/api/customerPayments?month=${date.month}&year=${date.year}`);
    ...
    <main className="bg-base_two">
      <div className="max-w-4xl mx-auto p-8 bg-base_color text-base_color font-semibold shadow-md my-10">
        <div className="flex justify-between items-center mb-5">
          <h2 className="text-2xl font-bold text-white">Customer Payment List</h2>
          <h2 className="text-xl font-bold text-white">{`${monthNames[date.month - 1]} ${date.year}`}</h2>
        </div>
          ...
          {entries.length === 0 ? (
            ...
            entries.map((entry) => (
              <li key={entry._id} className="mb-2">
                <Link
                  href={`/viewloadingandpayment/payment/${entry._id}`}
